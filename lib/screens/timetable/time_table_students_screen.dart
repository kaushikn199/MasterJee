...
class _TimeTableStudentsScreenState extends State<TimeTableStudentsScreen> {
  ...
  void didChangeDependencies() {
    if (!isCallFirst) {
      ...
      final timetableId = args?['timetableId']?.toString() ?? '';
      final subjectGroupId = args?['subjectGroupId']?.toString() ?? '';
        ...
        onPressed: () {
          setState(() {
            ...
            for (int i = 0; i < timetableStudentsList.length; i++) {
              if (timetableStudentsList[i].selectedValue == 1) {
                count = count + 1;
              }
            }
           if (count == 0) {
                        ...
                        children: [
                          Expanded(
                            child: buildRadioOption(
                              ...
                              value: 1,
                              groupValue:
                              timetableStudentsList[index].selectedValue ?? 1,
                              onChanged: (value) {
                                setState(() {
                          ...
                          Expanded(
                            child: buildRadioOption(
                              ...
                              value: 2,
                              groupValue:
                              timetableStudentsList[index].selectedValue ?? 0,
                              onChanged: (value) {
                                setState(() {
                                  timetableStudentsList[index].selectedValueText =
                                      leave;
                                  timetableStudentsList[index].selectedValue =
                                  value!;
                                });
                              },
                            ),
