...
class ParentLoginScreen extends StatefulWidget {
  ...
  static String routeName = 'parentLoginScreen';
...
class _ParentLoginScreenState extends State<ParentLoginScreen> {
  ...
  @override
  void initState() {
    callApiParentLoginReports();
    super.initState();
  }

  Future<void> callApiParentLoginReports() async {
    setState(() {
      _isLoading = true;
    });
    try {
      Hostelreportsresponse data =
      await Provider.of<ReportsApi>(context, listen: false).parentLoginReports(
      ...
      print("callApiParentLoginReports_error : $error");
      setState(() {
        _isLoading = false;
      });
    }
    ...
    return SingleChildScrollView(
      ...
      child: DataTable(
        ...
        columns: <DataColumn>[
          ...
          DataColumn(
              label: CommonText.semiBold("Parent Username",
                  size: 12.sp, color: colorBlack, overflow: TextOverflow.fade)),
          DataColumn(
              label: CommonText.semiBold("Parent Password",
                  size: 12.sp, color: colorBlack, overflow: TextOverflow.fade)),
        ],
          ...
          return DataRow(
            cells: [
              DataCell(CommonText.medium(
                  ...
                  color: colorBlack,
                  overflow: TextOverflow.fade)),
              DataCell(CommonText.medium(data.loginInfo?.username ?? "",
                  size: 11.sp,
                  color: colorBlack,
                  overflow: TextOverflow.fade)),
              DataCell(CommonText.medium("${data.loginInfo?.password ?? ""}",
                  size: 11.sp,
                  color: colorBlack,
                  overflow: TextOverflow.fade)),
            ],
